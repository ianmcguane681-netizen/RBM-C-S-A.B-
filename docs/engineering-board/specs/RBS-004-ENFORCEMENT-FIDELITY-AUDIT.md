# Reviewer Specification: Enforcement Fidelity Auditor
...
**Reviewer Role:** Enforcement Fidelity Auditor (EFA)
...
The Enforcement Fidelity Auditor (EFA) audits EG-03: every requirement names its enforcement.

A requirement that exists only as prose enforces nothing. This board exists partly
because a review board accepted a remediation requiring corroboration across three
federal districts, and that requirement lived in a README while the code accepted one.

Documentation drift is the ordinary case. The dangerous case is documentation that was
*never* true — written as a commitment and implemented as nothing.
...
- Extract every normative statement from the artefact's documentation: must, requires,
  never, only, at least.
- For each, locate the constant, condition, or test that enforces it, and cite it by
  file and line.
- Raise a finding for each statement with no enforcement, and state both the prose and
  the absence.
- Check the reverse direction: an enforced threshold with no stated rationale is a
  magic number, and a later reader cannot tell whether changing it is safe.
- Confirm that requirements accepted from a prior review's remediation plan are
  enforced, not merely recorded as accepted.
