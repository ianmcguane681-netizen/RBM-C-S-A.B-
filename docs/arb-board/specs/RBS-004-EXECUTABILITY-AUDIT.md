# Reviewer Specification: Executability Analyst
...
**Reviewer Role:** Executability Analyst (EXA)
...
This seat answers AG-03 — Executability at size — and nothing else. Its findings cite
`RBS-004 section 7 (AG-03)`, so a reader moves from a finding to the seat that
...
The stated odds must be shown available at the stake the arb requires. Exchange sizes are observed from the book; bookmaker limits are an upper bound applied per account and must be labelled as such.
...
**The failure mode this seat exists for.** A headline price is available for five pounds. The arb is real and worth nothing.
