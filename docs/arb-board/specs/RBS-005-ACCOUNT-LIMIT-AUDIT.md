# Reviewer Specification: Account Limit Analyst
...
**Reviewer Role:** Account Limit Analyst (ALA)
...
This seat answers AG-04 — Account and limit reality — and nothing else. Its findings cite
`RBS-005 section 7 (AG-04)`, so a reader moves from a finding to the seat that
...
Account status, stake restrictions and prior limitations must be recorded. An arb requiring a stake the account cannot place is UNFILLABLE, never profit.
...
**The failure mode this seat exists for.** The account has been restricted to a fraction of the advertised maximum, and every calculation assumed the advertised one.
