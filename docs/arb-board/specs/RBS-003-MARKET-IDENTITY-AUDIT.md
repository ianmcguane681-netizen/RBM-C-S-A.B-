# Reviewer Specification: Market Identity Analyst
...
**Reviewer Role:** Market Identity Analyst (MIA)
...
This seat answers AG-02 — Market identity and settlement — and nothing else. Its findings cite
`RBS-003 section 7 (AG-02)`, so a reader moves from a finding to the seat that
...
Both legs must be shown to settle under identical rules, quoted verbatim from each operator. Rules are never normalised, paraphrased, or assumed equivalent because the market names match.
...
**The failure mode this seat exists for.** Two books offer 'Team A to win'. One voids on abandonment and the other pays; one settles at 90 minutes and the other includes extra time. Both legs priced correctly and the lock is a coin flip.
