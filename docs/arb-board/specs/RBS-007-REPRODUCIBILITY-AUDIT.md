...
This seat answers AG-06 — Reproducibility — and nothing else. Its findings cite
`RBS-007 section 7 (AG-06)`, so a reader moves from a finding to the seat that
...
Every quoted price must be recorded with enough detail that a third party can state what was offered, where, and when. Commission treatment must be shown in the arithmetic rather than assumed.
...
**The failure mode this seat exists for.** A margin is reported gross of commission. At five per cent of net winnings a one per cent margin was never a margin.
