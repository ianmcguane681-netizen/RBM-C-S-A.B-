# Reviewer Specification: Share and Dilution Analyst
...
**Reviewer Role:** Share and Dilution Analyst (SDA)
...
This seat answers SG-03 — Share count and dilution — and nothing else. Its findings cite
`RBS-004 section 7 (SG-03)`, so a reader moves from a finding to the seat that
...
Basic and diluted counts must be distinguished, the count must be dated, and instruments issuable into shares must be enumerated. A market capitalisation must state which count it used.
...
**The failure mode this seat exists for.** Apple's FY2013 share count is filed at 899,213,000, then 899,213, then 6,294,494,000 after a retroactive split -- three answers for one period across a factor of 7,000, each equally confident.
