# Reviewer Specification: Market Liquidity Analyst
...
**Reviewer Role:** Market Liquidity Analyst (MLA)
...
This seat answers SG-05 — Exit liquidity — and nothing else. Its findings cite
`RBS-006 section 7 (SG-05)`, so a reader moves from a finding to the seat that
...
Liquidity must be assessed as the cost of exiting a stated position size, not as average daily volume. Borrow availability and cost are identified separately where a short is contemplated.
...
**The failure mode this seat exists for.** Average daily volume is reported as though it were depth. The question a holder faces is what leaving costs, and ADV does not answer it.
