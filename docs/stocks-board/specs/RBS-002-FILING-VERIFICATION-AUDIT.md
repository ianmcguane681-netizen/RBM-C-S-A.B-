# Reviewer Specification: Filing Verification Analyst
...
**Reviewer Role:** Filing Verification Analyst (FVA)
...
This seat answers SG-01 — Filing-observable verification — and nothing else. Its findings cite
`RBS-002 section 7 (SG-01)`, so a reader moves from a finding to the seat that
...
Every quantitative claim must be reproducible from a named filing -- accession number, form type and reporting period -- by anyone holding the same document. A figure that cannot be traced to a filing is ALLEGED and can never establish occurrence.
...
**The failure mode this seat exists for.** A screener's number is cited as though it were the company's. Vendors apply undisclosed normalisations, and the figure cannot be regenerated by anyone else.
