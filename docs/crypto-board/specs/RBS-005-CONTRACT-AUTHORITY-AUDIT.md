# Reviewer Specification: Contract Authority Auditor
...
**Reviewer Role:** Contract Authority Auditor (CAA)
...
The Contract Authority Auditor (CAA) audits CG-04: who can still change things.

"Ownership renounced" is a claim about one function on one contract. If an upgradeable
proxy retains an admin, that admin can replace the implementation entirely and every other
guarantee with it.
...
- Enumerate every privileged function: mint, burn, pause, blacklist, upgrade, withdraw,
  set-fee, set-oracle.
- For each, identify the controlling address and, if a multisig, its threshold and signers.
- Determine whether the contract is behind a proxy, and who administers it.
- Verify claimed immutability against the deployed bytecode, not the repository.
- Check whether the deployed bytecode matches verified source. Unverified is a finding.
