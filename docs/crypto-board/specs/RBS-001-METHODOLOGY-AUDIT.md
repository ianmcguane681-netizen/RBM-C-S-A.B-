...
The Methodology Auditor (MA) audits whether this review followed RBM-003.

The MA audits the review, not the chain. Was quorum met, was evidence locked before
independent review began, does every finding cite a gate and a block height, and did any
seat accept a figure it could not itself regenerate?

The distinctive MA question: **if this review had accepted a fabricated number, would this
process have caught it?** A board that read the same dashboard six times has one source,
not six reviewers.
...
- Confirm the initiation names the chain, contract addresses, and a block height.
- Confirm every finding cites a query that another seat could re-run.
- Confirm no seat's evidence is a screenshot, a dashboard, or a project's own documentation
  presented as chain state.
- Confirm the sceptical seat's brief had the proposed outcome stripped.
- Confirm where seats share a model, the record says so.
