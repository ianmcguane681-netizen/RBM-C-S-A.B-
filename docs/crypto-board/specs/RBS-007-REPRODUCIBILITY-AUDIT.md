...
The Reproducibility Auditor (RPA) audits CG-06: the figures regenerate.

Crypto's evidentiary advantage is that claims are cheap to re-establish -- and it is
forfeited the moment a figure is recorded without the block height that produced it.

A number that was correct when taken and cannot be regenerated a week later is a claim.
...
- Confirm every figure in every seat's report records chain, block height, contract address
  and query.
- Re-run a sample of queries against an archive node and confirm the figures match.
- Confirm no figure depends on a proprietary endpoint that another reviewer could not reach.
- Record the node or provider used, and whether it serves archive state.
- Confirm the review names the block height as its artefact version, so the whole review is
  pinned to a point in chain history.
