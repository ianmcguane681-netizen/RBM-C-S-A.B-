"""Fire every lane that is due, hold the ones that are not, and show what awaits you.
    ...
    python run.py --reap [lane]   all three reaper lanes, or one of arb/crypto/stocks
...
if __name__ == "__main__":
    ...
    if "--reap" in argv:
        rest = argv[argv.index("--reap") + 1:]
        raise SystemExit(reap(rest[0] if rest else ""))
    if "--queue" in argv:
